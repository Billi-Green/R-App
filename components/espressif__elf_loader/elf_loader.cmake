...
macro(project_so project_name)
    ...
    # The Windows ESP RISC-V toolchain can require plain "as" for custom
    # shared-object compilation even though GCC is prefixed. Use an explicitly
    # supplied real assembler when available; some recent toolchains ship the
    # prefixed executable as a wrapper that cannot be renamed to "as.exe".
    if(CMAKE_HOST_WIN32 AND CMAKE_C_COMPILER MATCHES "riscv32-esp-elf-gcc")
        get_filename_component(elf_toolchain_bin "${CMAKE_C_COMPILER}" DIRECTORY)
        set(elf_assembler "")
        if(DEFINED ELF_ASSEMBLER)
            set(elf_assembler "${ELF_ASSEMBLER}")
        elseif(DEFINED ENV{ELF_ASSEMBLER})
            set(elf_assembler "$ENV{ELF_ASSEMBLER}")
        elseif(EXISTS "${elf_toolchain_bin}/as.exe")
            set(elf_assembler "${elf_toolchain_bin}/as.exe")
        endif()
        set(elf_assembler_alias_dir "${CMAKE_BINARY_DIR}/toolchain_alias")
        set(elf_assembler_alias "${elf_assembler_alias_dir}/as.exe")
        if(elf_assembler AND EXISTS "${elf_assembler}")
            file(MAKE_DIRECTORY "${elf_assembler_alias_dir}")
            configure_file("${elf_assembler}" "${elf_assembler_alias}" COPYONLY)
            list(APPEND so_compile_flags "-B${elf_assembler_alias_dir}/")
        endif()
    endif()
